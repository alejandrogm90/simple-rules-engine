...
def export_type(cls):
    """ Decorator to expose the given class to simple_rules_engine.export_rule_data. """
...
class StringType(BaseType):
    ...
    def contains(self, other_string):
        if other_string.find(self.value) == -1:
            return False
        return True

    @type_operator(FIELD_TEXT)
    def not_contains(self, other_string):
        return not self.contains(other_string)
...
class NumericType(BaseType):
    ...
    @staticmethod
    def _assert_valid_value_and_cast(value) -> Decimal:
