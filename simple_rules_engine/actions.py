        ...
        for param in params:
            ...
            if param_name not in func.__code__.co_varnames:
                raise AssertionError(
                    "Unknown parameter name {0} specified for action {1}".format(param_name, func.__name__))

            if field_type not in valid_fields:
                raise AssertionError(
                    "Unknown field type {0} specified for action {1} param {2}".format(field_type, func.__name__,
                                                                                       param_name))
...
def rule_action(label=None, params=None):
    def wrapper(func):
        ...
        if isinstance(params, dict):
            params_ = [dict(label=fn_name_to_pretty_label(name), name=name, fieldType=field_type) for name, field_type
                       in params.items()]
