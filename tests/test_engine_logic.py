...
from simple_rules_engine import engine
from simple_rules_engine.actions import BaseActions
from simple_rules_engine.operators import StringType
from simple_rules_engine.variables import BaseVariables
...
class EngineTests(TestCase):
    ...
    def test_check_all_condition_with_no_items_fails(self):
        with self.assertRaises(AssertionError):
            engine.check_conditions_recursively({
                'all': []
            }, BaseVariables())
    ...
    def test_nested_all_and_any(self, *args):
        ...
        engine.check_condition.assert_any_call({
            'name': 1
        }, bv)
        engine.check_condition.assert_any_call({
            'name': 2
